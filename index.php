...
//			$stated_as = $article->authors_stated_as[$qt] ;
		...
		foreach ( $article->published_in AS $qt ) {
			...
			$published_in[] = wikidata_link($qt, $label, 'black') . "&nbsp;[<a href='https://qlever.scholia.wiki/venue/$qt/missing' target='_blank'>missing</a>]" ;
		}
		...
		if ( count($article->topics) > 0 ) {
			...
			foreach ( $article->topics AS $qt ) {
				...
				$topics[] = wikidata_link($qt, $label, 'brown') . "&nbsp;[<a href='https://qlever.scholia.wiki/topic/$qt/missing' target='_blank'>missing</a>]" ;
			}
...
foreach ( $potential_author_data AS $q => $author_data ) {
	...
	foreach ( $author_data->employer_qids AS $emp_qid ) {
		...
		print wikidata_link($emp_qid, $emp_item->getLabel(), '') . "&nbsp;[<a href='https://qlever.scholia.wiki/organization/$emp_qid/missing' target='_blank'>missing</a>]<br/>" ;
	}
	print "</td></tr>" ;
}
