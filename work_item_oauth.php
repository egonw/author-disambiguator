...
print ' | ' ;
print "<a target='_blank' href='https://qlever.scholia.wiki/work/$work_qid'>Scholia Work Page</a>&nbsp;[<a href='https://qlever.scholia.wiki/work/$work_qid/curation' target='_blank'>curation</a>]";
print ' | ' ;
...
foreach ( $article_entry->published_in AS $qt ) {
	$i2 = $wil->getItem ( $qt ) ;
	if ( isset($i2) ) $published_in[] = wikidata_link($i2->getQ(), $i2->getLabel(), 'black') . "&nbsp;[<a href='https://qlever.scholia.wiki/venue/" . $i2->getQ() . "/curation' target='_blank'>curation</a>]" ;
}
...
if ( count($article_entry->topics) > 0 ) {
	...
	foreach ( $article_entry->topics AS $qt ) {
		...
		$topics[] = wikidata_link($i2->getQ(), $i2->getLabel(), 'brown') . "&nbsp;[<a href='https://qlever.scholia.wiki/topic/" . $i2->getQ() . "/curation' target='_blank'>curation</a>]" ;
	}
...
foreach ( $article_entry->authors AS $num => $qt_list ) {
	...
	foreach ( $qt_list AS $id => $qt ) {
		...
		$label = $i2->getLabel() ;
		$formatted_authors[$num][$id] = "<a href='author_item_oauth.php?limit=50&id=" . $i2->getQ() . "' style='color:green'>$label</a>&nbsp;[<a href='https://qlever.scholia.wiki/author/" . $i2->getQ() . "/curation' target='_blank'>curation</a>]" ;
